...
- **粘贴截图分析时**：截图里「我」说的每一句都是你真实说过的话，会连同上文收进 `my_style.jsonl`。等你点「分析并排序」才收，所以你有时间先用 ⇄ 和 × 把识别错的改好。只认截图来的内容，手打的演示文字不会被当成你的话
- **自己写候选、点复制时**：那几条本来就是你亲手写的，你选中的那条会加进语料，界面上会提示「已加进你的语料（N 条）」

重复的句子不会重复记录。用得越久，AI 越像你。
